...
const SKILLS: Skill[] = [
  {
    ...
    modalId: 'modal-html5',
    category: 'Frontend',
    content: `시맨틱 태그를 사용하여 문서의 구조와 의미를 명확히 표현할 수 있습니다. 검색 엔진 최적화(SEO)에 유리하고, 코드의 가독성이 향상되며, 스크린 리더와 같은 보조 기술이 콘텐츠를 더 잘 이해할 수 있어 웹 접근성이 높아지며 이를 프로젝트에 적용한 경험이 있습니다.
  ...
  },
  {
    ...
    ES6에서의 arrow functions, class, template literals, destructuring assignments, default parameters, Map, Set, Promises 등 이러한 기능들을 활용하여 더 간결하고 효율적인 코드를 작성할 수 있습니다.
  ...
  },
  {
    ...
    React에서 상태 관리의 중요성을 깨닫고, 컴포넌트 내에서 상태(state)를 효율적으로 관리하는 방법을 학습했습니다. 이를 통해 UI의 변화를 즉시 반영하고, 사용자와의 상호작용에 따라 동적으로 변하는 화면을 구현할 수 있었습니다. 상태 변화에 따른 리렌더링을 적절히 활용하여 성능을 최적화하고, 복잡한 애플리케이션에서 상태 흐름을 명확히 관리할 수 있었습니다. 예를 들어, 상태를 로컬 상태와 전역 상태로 나누어 관리하거나, 상태 관리 라이브러리인 Zustand를 활용하여 상태를 중앙에서 일관되게 관리함으로써, 컴포넌트 간의 상태 공유와 유지보수를 용이하게 했습니다. 또한, useState, useReducer, useContext와 같은 훅을 적절히 사용하여 컴포넌트의 상태를 효율적으로 관리하고, 필요에 따라 비동기 상태 관리를 위해 React Query를 활용하여 데이터 페칭과 캐싱을 최적화를 할 수 있었습니다.
  ...
  },
  {
    ...
    category: 'Frontend',
    content: `Zustand를 활용하여 전역 상태 관리를 단순하고 직관적으로 구현했습니다. 복잡한 상태 관리 로직을 간소화하고, 성능 최적화를 통해 효율적인 애플리케이션을 만들 수 있었습니다. 이를 통해 상태 관리의 복잡성을 줄이고, 프로젝트에서 실제로 적용하여 효과를 보았습니다.
    `,
  },
  {
    name: 'TailwindCss',
